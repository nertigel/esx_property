...
function RemoveOwnedProperty(name, owner)
    ...
    local result = MySQL.Sync.fetchAll('SELECT rented FROM owned_properties WHERE name = @name AND owner = @owner',
    {
        ['@name'] = name,
        ['@owner'] = owner
    })
    
    if tonumber(result[1].rented) == 0 then
        local xPlayer = ESX.GetPlayerFromId(source)
        
        local getSellPrice = MySQL.Sync.fetchAll('SELECT price FROM properties WHERE name = @name',
    {
        ['@name'] = name
    })
        local cost = tonumber(getSellPrice[1].price)
        local lowEnd = cost/2
        local highEnd = cost*1.5
        local payment = math.random(lowEnd,highEnd)
        
        xPlayer.addAccountMoney('bank', payment)
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'Bank recieved ~g~$'.. tostring(payment) ..' ~w~ from selling home')
    end
    ...
    MySQL.Async.execute('DELETE FROM owned_properties WHERE name = @name AND owner = @owner',
    {
        ...
        if xPlayer then
            TriggerClientEvent('esx:showNotification', xPlayer.source, 'Property no longer owned')
        end
    end)
end
...
AddEventHandler('esx_property:removeOwnedProperty', function(propertyName)
	...
	MySQL.Async.fetchAll('SELECT price FROM owned_properties', {}, function(result)
